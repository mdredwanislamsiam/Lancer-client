		...
		<div className="card bg-base-100 shadow-md hover:shadow-lg transition-shadow border border-base-200 rounded-xl overflow-hidden mt-2">
			<div className="card-body">
				<div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
					<div>
						<Link to={`/infoPage/${review.user.id}`} className="flex gap-2 justify-center items-center my-2 cursor-pointer">
							<img src={review.user.image || defImg} alt="" className="w-8 h-8 rounded-full shadow-sm" />
							<p className="font-semibold my-2">{review.user.username}</p>
						</Link>
							...
							<div className="flex text-yellow-400">
								{[...Array(5)].map((_, i) => (
									<FaStar
										key={i}
										size={20}
										className={i < review.ratings ? "text-yellow-500" : "text-gray-300"}
									/>
								))}
							</div>
						</div>
				...
				{isEditing ?
					...
					/>
				:	<div className="mt-4 bg-blue-200 py-1 px-10 rounded-lg shadow-lg text-black">
						<p className="leading-relaxed whitespace-pre-line break-words">{review.comment}</p>
					</div>
				}
			</div>
		</div>
