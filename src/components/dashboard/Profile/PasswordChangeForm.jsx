...
const PasswordChangeForm = ({ register, errors, watch, isEditing }) => {
    ...
    return (
		<div>
			<button
				type="button"
				className="btn btn-xs lg:btn-md btn-link p-0 justify-start font-semibold"
				onClick={() => setIsPasswordSectionOpen(!isPasswordSectionOpen)}>
				Change Password
			</button>
			{isPasswordSectionOpen && (
				<div className="mt-3 space-y-3 pl-2 border-l-2 border-base-300">
					<div className="form-control text-xs lg:text-sm">
						...
						<div className="relative">
							<input
								...
								className="input input-bordered text-xs lg:text-sm bg-base-200 w-full outline-none pr-10"
								disabled={!isEditing}
