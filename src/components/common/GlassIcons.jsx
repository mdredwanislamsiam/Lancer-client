...
const GlassIcons = ({ items, className }) => {
	...
	return (
		<div
			className={`grid gap-[2em] md:gap-[5em] grid-cols-2 md:grid-cols-6 mx-auto py-[3em] overflow-visible ${className || ""}`}>
			{items.map((item, index) => (
				<button
					key={index}
					type="button"
					aria-label={item.label}
					className={`relative bg-transparent outline-none mx-auto border-none cursor-pointer w-[10em] h-[8em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] shadow-xl rounded-full group ${
						item.customClass || ""
					}`}>
					<span
						className="absolute top-0 left-0 w-full h-full rounded-[1.25em] block transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] origin-[100%_100%] rotate-[0deg] [will-change:transform] group-hover:[transform:rotate(0deg)_translate3d(0,-1em,0.5em)] bg-[#0F4C75]"
						style={{
